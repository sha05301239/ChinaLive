...
class XZSearchCollectionCell: UICollectionViewCell {
    ...
    lazy var titleLabel : UILabel = {
        let lb = UILabel.init()
        lb.font = UIFont.systemFont(ofSize: ddSpacing(27))
        lb.textAlignment = .center
        lb.backgroundColor = ddRandomColor()
        return lb
    }()
...
//MARK:--collectionView点击item方法
protocol XZSearchCollectionViewDelegate {
    /***/
...
class XZSearchCollectionView: UIView {
    ...
    var delegate : XZSearchCollectionViewDelegate?
    
    var dataList:Array<XZSearchModel>?{
        didSet{
...
//        layout.itemSize = CGSize(width:itemW , height: itemW*220/350)
        layout.minimumLineSpacing = ddSpacing(20);
        layout.minimumInteritemSpacing = ddSpacing(15)
        layout.sectionInset = UIEdgeInsets(top: 10, left: ddSpacing(30), bottom: 0, right: ddSpacing(25))
...
extension XZSearchCollectionView:UICollectionViewDelegateFlowLayout,UICollectionViewDataSource{
    ...
    func collectionView(_ collectionView: UICollectionView, layout collectionViewLayout: UICollectionViewLayout, sizeForItemAt indexPath: IndexPath) -> CGSize {
        ...
        DDLog("self.dataList = \(String(describing: self.dataList))")
        let model = self.dataList![indexPath.item  ]
        
        let itemW = getLabelWidth(str: model.keyWord ?? "", fontSize: ddSpacing(27), height: ddSpacing(44))
        
        return CGSize(width: itemW+ddSpacing(30), height: ddSpacing(44))
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        delegate.hisCollectionDidSelectItemAt(indexPath)
